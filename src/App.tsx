...
export default function App() {
  ...
  const analysis=useMemo(()=>{if(!bounds)return {result:null,error:''};try{const result=analyzeLayers(models,bounds);const missing=configs.filter(c=>c.visible&&c.opacity>0&&!models.some(m=>m.id===c.id));if(missing.length)result.warnings.push('Resultado parcial: faltan capas visibles sin cargar: '+missing.map(c=>c.name).join(', ')+'. Revisa sus errores en Capas.');if(loading)result.warnings.push('La carga de capas no ha terminado; el resultado puede cambiar.');return {result,error:''};}catch(error){return {result:null,error:String(error instanceof Error?error.message:error)};}},[models,bounds,configs,loading]);
  ...
  const startMode=(next:MapMode)=>{setInteractionId(i=>i+1);setMode(next);if(next){setSelection(null);if(window.matchMedia('(max-width: 800px)').matches)setOpen(false);}};
  ...
  return <div className={`app ${open?'panel-open':'panel-closed'} ${mode?'interacting':''}`} style={{'--panel-width':`${width}px`} as React.CSSProperties}>
    ...
    <aside className="sidebar" aria-label="Panel de herramientas" hidden={!open}><header><div><span className="eyebrow">ATLAS DE TRABAJO</span><h1>Explorar el territorio</h1><p>Chile · Asturias · tus datos</p></div><button className="close-panel" aria-label="Plegar panel" onClick={()=>setOpen(false)}>‹</button></header>
      ...
      <section hidden={tab!=='analysis'}><AnalysisPanel layers={models} bounds={bounds} selecting={mode==='rectangle'} onStart={()=>startMode('rectangle')} onClear={()=>{setBounds(null);setMode(null);}} result={analysis.result}/>{analysis.error&&<Alert type="error" message={analysis.error}/>}</section>
