...
- **Análisis:** dibuja un rectángulo arrastrando o marcando dos esquinas, o elige **Libre por puntos**. En modo libre marca al menos tres vértices y toca el primero o pulsa **Cerrar y analizar**. Hasta cerrar no se calculan resultados. Puedes deshacer puntos; los contornos cruzados se rechazan. Incluye entidades que intersectan su geometría real, también el borde. Cada Feature cuenta una vez, incluso MultiPoint, MultiLineString y MultiPolygon. Calcula km²/ha, recuentos, estadísticas y medidas recortadas. Exporta JSON, CSV o entidades GeoJSON completas. [Criterio y límites](docs/ANALISIS.md).
